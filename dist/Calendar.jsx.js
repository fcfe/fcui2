...
define(function (require) {
    ...
    return React.createClass({
        ...
        onDayClick: function onDayClick(e) {
            ...
            var timer = tool.str2date(this.state.displayYear + '-' + (this.state.displayMonth + 1) + '-' + e.target.value);
        ...
        // 操作区右侧按钮被点击
        onMonthAdd: function onMonthAdd(e) {
            if (this.props.disabled) return;
            var month = this.state.displayMonth;
            var year = this.state.displayYear;
            month = month * 1 + 1;
        ...
        // 操作区左侧按钮被点击
        onMonthSub: function onMonthSub(e) {
            if (this.props.disabled) return;
            var month = this.state.displayMonth;
            var year = this.state.displayYear;
            month = month * 1 - 1;
        ...
        render: function render() {
            return React.createElement(
                'div',
                cTools.containerBaseProps('calendar', this),
                this.context.appSkin !== 'oneux4' ? factory.normalOperationBarFactory(this) : factory.oneux4OperationBarFactory(this),
                React.createElement(
                    'div',
                    { className: 'calendar-day-label' },
                    this.state.inRange ? language.calendar.day.map(factory.dayLabelFactory) : this.props.min.replace(/-/g, '.') + ' - ' + this.props.max.replace(/-/g, '.')
                ),
                React.createElement(
                    'div',
