...
define(function (require) {
    ...
    var exports = {
        ...
        isEqual: function isEqual(x, y) {
            ...
            function compare2Objects(x, y) {
                ...
                for (p in y) {
                    ...
                    if (_typeof(y[p]) !== _typeof(x[p])) return false;
                }
                for (p in x) {
                    if (y.hasOwnProperty(p) !== x.hasOwnProperty(p)) return false;
                    if (_typeof(y[p]) !== _typeof(x[p])) return false;
         ...
         * 获取浏览器厂商
         */
        getBrowserEnterprise: function getBrowserEnterprise() {
            // in NodeJS
            if (!(typeof window !== 'undefined' && window.document && window.document.createElement)) {
                return 'chrome';
            }
         ...
         * @return {Object} 命名空间
         */
        getNamespace: function getNamespace(namespace) {
         ...
         */
        getCursorPosition: function getCursorPosition(dom) {
            var result = -1;
            // 非IE浏览器
         ...
         */
        setCursorPosition: function setCursorPosition(dom, pos) {
            // 非IE浏览器
         ...
         * @return {Boolean} 检查结果
         */
        isDOMVisible: function isDOMVisible(dom) {
            if (!dom) return false;
            var initDOM = dom;
         ...
         */
        getDOMPosition: function getDOMPosition(e) {
         ...
         */
        dateFormat: function dateFormat(date, tpl) {
            //author: meizz
