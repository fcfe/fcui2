...
define(function (require) {
    ...
    return {
         ...
         */
        getCursorSize: function getCursorSize(state) {
            var pos = {
                left: -2,
                top: -2,
                width: 0,
            ...
            var axis1 = this.getGridAxis(Math.min(state.mouseDownX, state.mouseCurrentX), Math.min(state.mouseDownY, state.mouseCurrentY));
            var axis2 = this.getGridAxis(Math.max(state.mouseDownX, state.mouseCurrentX), Math.max(state.mouseDownY, state.mouseCurrentY));
         ...
         */
        getGridAxis: function getGridAxis(x, y) {
         ...
         */
        getMergedValues: function getMergedValues(arr) {
            var result = [];
            var beginIndex = 0;
            var prevValue = null;
         ...
         */
        getSelectedRange: function getSelectedRange(state) {
            var axis1 = this.getGridAxis(Math.min(state.mouseDownX, state.mouseCurrentX), Math.min(state.mouseDownY, state.mouseCurrentY));
            var axis2 = this.getGridAxis(Math.max(state.mouseDownX, state.mouseCurrentX), Math.max(state.mouseDownY, state.mouseCurrentY));
            return {
                startHour: axis1.x,
         ...
         */
        getRangeSelectedCount: function getRangeSelectedCount(value, axis1, axis2) {
         ...
         */
        getRangeIntro: function getRangeIntro(startHour, endHour, startWeekday, endWeekday) {
            if (isEmpty(startHour)) return '';
            var res = '';
            res += isEmpty(startWeekday) ? '' : language.day[startWeekday];
            res += isEmpty(endWeekday) || startWeekday === endWeekday ? '' : (res.length ? ' - ' : '') + language.day[endWeekday];
            res += res.length ? ' ' : '';
         ...
         * @param {Number} param.mouseCurrentY 鼠标当前y坐标
         */
        updateValueByMouse: function updateValueByMouse(value, state) {
