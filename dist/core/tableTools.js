...
define(function (require) {
    ...
    return {
         ...
         */
        getSelectedCount: function getSelectedCount(obj) {
            if (obj === -1) return -1;
            var n = 0;
            for (var key in obj) {
         ...
         */
        getSelectedHash: function getSelectedHash(value) {
            value = typeof value === 'string' ? JSON.parse(value) : {};
            var selected = {};
            if (value.selected === -1) {
                selected = -1;
            } else if (value.selected instanceof Array) {
                for (var i = 0; i < value.selected.length; i++) {
                    selected[value.selected[i]] = true;
                }
            }
            return selected;
        },
         ...
         */
        getRowSelectedState: function getRowSelectedState(row, value) {
            var selectHash = this.getSelectedHash(value);
            var indeterminate = {};
            value = typeof value === 'string' ? JSON.parse(value) : {};
            if (value.indeterminate instanceof Array) {
                for (var i = 0; i < value.indeterminate.length; i++) {
                    indeterminate[value.indeterminate[i]] = true;
                }
            }
         ...
         */
        updateSelected: function updateSelected(index, checked, selectMode, tableValue, datasource) {
            var result = [];
         ...
         */
        tdPropsFactory: function tdPropsFactory(conf, item, me, row, column) {
            // 深度克隆fieldConfig
         ...
         * @return {Array} 列配置
         */
        fieldConfigFactory: function fieldConfigFactory(me, renderers) {
            var fields = [];
            ...
            var selectorConfig = {
                ...
                prepare: function prepare(props, item, row, column, me) {
